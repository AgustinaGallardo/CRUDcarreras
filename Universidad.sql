...
--Creacion de SP (Carreras que se cursan en x año | x se ingresara por parametro)
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[SP_Carreras]
@anio INT
AS
	SELECT C.nombre 'Carrera',  C.titulo 'Tíulo', YEAR(anioCursado) 'Año de cursado', M.nombre 'Materia'  
	FROM Carreras C JOIN DetalleCarreras D ON C.idCarrera=D.idCarrera
		 JOIN Materias M ON D.idMateria=M.idMateria
	WHERE YEAR(anioCursado)=@anio
	ORDER BY M.nombre
